...
class ImageAddViewController: UIViewController {
    ...
    @IBAction func btnAddItem(_ sender: UIButton) {
        if tfInsert.text!.trimmingCharacters(in: .whitespaces).isEmpty{
            ImageMessage.check = "Old"
        }else{
            ImageMessage.check = "New"
            ImageMessage.items = tfInsert.text!
            ImageMessage.itemsImageFiles = imageFileName[ImageMessage.rowNum]
        }
        
        tfInsert.text = ""
        imgView.image = imageArray[ImageMessage.rowNum]
